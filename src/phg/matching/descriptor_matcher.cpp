                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (int i = 0; i < n_matches; ++i) {
        ...
        int j = 0;
        while (neighbors_indices_query_i.size() < total_neighbours && distances2_query.at<float>(i, j) < radius2_query) {
            neighbors_indices_query_i.push_back(indices_query.at<int>(i, j));
            ++j;
        }
        j = 0;
        while (neighbors_indices_train_i.size() < total_neighbours && distances2_train.at<float>(i, j) < radius2_train) {
