...
//            std::cout << "(" << x0 << ", " << y0 << "), (" << x1 << ", " << y1 << ")" << std::endl;
        ...
        //берем последний столбец, так как он соответсвует нулевому сингулярному числу
        ...
        Eigen::Vector3d singular_values = svdf.singularValues();
        Eigen::DiagonalMatrix<double, 3> S(singular_values[0], singular_values[1], 0);
    ...
    {
        cv::Vec2d mean_pt = cv::Vec2d(0.0, 0.0);
        for (cv::Vec2d pt : m) {
            mean_pt += pt;
        }
        ...
        for (cv::Vec2d pt: m) {
            root_mean_square += cv::norm(pt - mean_pt);
        }
        root_mean_square /= m.size();

        double scale = sqrt(2) / root_mean_square;
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
        ...
        const int n_inliers = 0.5 * n_matches;
        const int n_samples = 8;

        float w = (float) n_inliers / n_matches;
