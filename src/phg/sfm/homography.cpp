...
//         * [3] http://ikrisoft.blogspot.com/2015/01/ransac-with-contrario-approach.html
        ...
        const int n_trials = (int) (log(1 - success_p) / log(1 - pow(w, n_samples)));
...
{
    cv::Vec3d homogeneous_point = cv::Vec3d(pt.x, pt.y, 1.0);
    cv::Mat transform_point = T * homogeneous_point;

    double x = transform_point.at<double>(0);
