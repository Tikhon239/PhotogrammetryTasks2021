...
void computeHomography(int cur_img_index, std::vector<bool> &visited, std::vector<cv::Mat> &Hs,
                       const std::vector<cv::Mat> &imgs,
                       const std::vector<int> &parent,
                       std::function<cv::Mat(const cv::Mat &, const cv::Mat &)> &homography_builder) {
    if (!visited[cur_img_index]) {
        visited[cur_img_index] = true;

        int parent_img_index = parent[cur_img_index];
        if (parent_img_index == -1) {
            Hs[cur_img_index] = cv::Mat::eye(3, 3, CV_64F);
        } else {
            computeHomography(parent_img_index, visited, Hs, imgs, parent, homography_builder);
            cv::Mat homography = homography_builder(imgs[cur_img_index], imgs[parent_img_index]);
            Hs[cur_img_index] = Hs[parent_img_index] * homography;
        }
    }
}
                            ...
                            std::function<cv::Mat(const cv::Mat &, const cv::Mat &)> &homography_builder)
{
    ...
    std::vector<cv::Mat> Hs(n_images);
    {
        ...
        std::vector<bool> visited(n_images);
        for (int i = 0; i < n_images; ++i) {
            computeHomography(i, visited, Hs, imgs, parent, homography_builder);
        }
    }
